...
func GenerateRandomDate() time.Time {
	var now = time.Now().In(rnokpp.BaseLocation)
	var randomMonth, randomDay int

	randomYear := mathRand.Intn(now.Year()-rnokpp.BaseYear+1) + rnokpp.BaseYear
	...
	return time.Date(randomYear, time.Month(randomMonth), randomDay, 0, 0, 0, 0, rnokpp.BaseLocation)
}
